...
Injector.prototype.inject = function(target) {
	...
	this._inject(target, realModule, new Dependency(target.name || 'anonymous'), [], noopWithError)
}
...
Injector.prototype._inject = function(target, realModule, targetDependency, ancestors, callback) {
	...
	for (var i = 0; i < params.length; i++) {
		...
		if (exists) {
			args.push(dependency.load(realModule, targetDependency))
...
function invokeTarget(injector, target, dependency, hasCallbackParam, callback) {
	return function invokeTargetFunctor(err, results) {
		...
		if (hasCallbackParam) {
			...
			resolve(null, target.apply(null, results))
		}
	}
}
