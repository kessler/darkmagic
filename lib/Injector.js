...
Injector.prototype.inject = function(target, context, _realModule /* internal */, depName, _callback /* internal */) {
	...
	for (var i = 0; i < params.length; i++) {
		...
		var resolveDependencyCallback = resolveDependencyCallbackFactory(this._registry, signaller, args, i, metadata, isNewDependency)
...
Injector.prototype.add = function (name, dependency) {
	throw new Error('not implemented yet, but really you should use inject()')
}
...
Injector.prototype.remove = function (name) {
	...
	if (metadata) {
		...
		delete require.cache[metadata.requireId]
	}
}

Injector.prototype.getMetadata = function(name) {
