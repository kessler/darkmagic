...
Injector.prototype.inject = function(target, callback) {
	...
	this._inject(target, realModule, new Dependency(target.name || 'anonymous'), [], handleError(this, callback))
}
...
function handleError(injector, userCallback) {
	return function handler(err) {

		if (err) {
			if (injector._events.error !== undefined) {
				debug('firing error event')
				injector.emit('error', err)
