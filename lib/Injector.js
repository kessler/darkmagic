...
function Injector(options) {
	...
	this.autoInjectLocalFactories = options.autoInjectLocalFactories || true
	this.autoInjectExternalFactories = options.autoInjectExternalFactories || true
}
...
Injector.prototype._inject = function(target, context, realModule, targetDependency, ancestors, callback) {
	...
	for (var i = 0; i < params.length; i++) {
		...
		if (isNewDependency) {
			dependency = this._newDependency(dependencyName)
		}
...
Injector.prototype._newDependency = function (name) {
