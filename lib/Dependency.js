...
Dependency.prototype.load = function (realModule, parentDependency) {
	...
	if (artifact && artifact.$darkmagic) {
		...
		if (this.isFactory && artifact.name && artifact.name.substr(0, 10) === 'dontInject') {
			this.isFactory = false
		} else if (this.isFactory && artifact.name && artifact.name[0] === artifact.name[0].toUpperCase() && isNaN(artifact.name[0])) {
